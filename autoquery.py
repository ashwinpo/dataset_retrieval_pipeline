...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        error_message = "Usage: python2.7 autoquery.py <question file> <output dir> <index type>.\n\n"
    ...
    output_dir_in = sys.argv[2] ## e.g. raw_metadata_auto_query
    ...
    for ind in range(len(ques_id)):
        ...
        ez = Entrez.esearch(db="pubmed",term=ques)
        for ret in ez:
            begin = ret.find("</TranslationStack><QueryTranslation>")
            if(begin != -1):
                end = ret.find("</QueryTranslation>")
                internal_query = pubmed_query_analyzer(ret[(begin+37):end])
    ...
    for ind in range(len(json_queries)):
        ...
        # f = open(os.path.join(out_dir, ques_id[ind]+"_hit_doc.txt"),'w')
        # for doc_id in hit_id_list:
        #     f.write("================================================================\n")
        #     f.write("document: "+doc_id+"\n")
        #     f.write(file(os.path.join(datamed_json_dir, doc_id+".json")).read())
        #     f.write("\n\n")
        # f.close()
        ...
        # ## Output queries
        # # with open("queries/"+ques_id[ind]+"_query.json",'w') as f:
        # with open(os.path.join(out_dir, ques_id[ind]+"_query.json"),'w') as f:
        #     json.dump(json_queries[ind],f,indent=4,sort_keys = True)
