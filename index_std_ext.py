...
print
add_doc_list = os.listdir(add_fields_dir)
add_doc_list = {doc:1 for doc in add_doc_list}
print type(add_doc_list), len(add_doc_list)
print
...
while start_idx<len(docList):
    ...
    for fname in subList:
        ...
        if add_doc_list.get(fname, None):
            try:
                ext = filter(None, file(os.path.join(add_fields_dir,fname)).read().split("\n"))
                ext = ". ".join(ext)

                if not all(ord(c) < 128 for c in ext):
                    ext = ''.join([i if ord(i) < 128 else ' ' for i in ext])
                action['_source']['extension'] = ext ## additional text
            except IOError, e:
                if e.errno==2:
                    pass
                else:
